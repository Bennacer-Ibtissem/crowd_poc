...
if uploaded_file is not None:
    ...
    if st.button("🚀 Start Processing"):
        ...
        with col2:
            ...
            if is_portrait:
                st.info(
                    "📱 Processing Portrait Video"
                    + (" (Auto-rotating)" if force_rotate else "")
                )
        ...
        while cap.isOpened():
            ...
            if frame_count % 5 == 0 or frame_count == total_frames:
                ...
                progress_text.markdown(
                    f"### Processing: {frame_count}/{total_frames} frames ({progress_percentage:.2f}%)"
                )

                # Display real-time metrics
                avg_fps = sum(fps_list[-20:]) / min(
                    len(fps_list), 20
                )  # Rolling average
                ...
                metrics_placeholder.dataframe(
                    pd.DataFrame(metrics_data), hide_index=True
                )
